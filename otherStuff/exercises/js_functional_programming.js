...
function exercise2() {
  ...
  courses.forEach((course) => idNameArray.push({ "id": course.id, "name": course.name }));
  return idNameArray;
}
...
function exercise5() {
  ...
  const timeFilter = (course) => course.hours == 3;
  return courses.filter(timeFilter)
}
...
function exercise6() {
  ...
  const timeFilter = (course) => course.hours == 3;
  const teacherMapping = (course) => course.teacher;
  return courses.filter(timeFilter)
                  .map(teacherMapping);
}
...
function exercise7() {
  ...
  let courseWithLargestId;
  courses.forEach(function(course){
    if(courseWithLargestId == undefined || courseWithLargestId.id < course.id){
      courseWithLargestId = course;
    }
  });
...
function exercise9() {
  ...
  //return courses.reduce((acc, current) => acc + current.hours, 0)
  const sum = (acc, current) => acc + parseInt(current.hours);
...
function exercise11() {
  ...
  return impacts.filter((impact) => impact.geolocation != undefined).length;
}

function exercise12() {
  // return an array with the names of all the impacts that have no geolocation
  return impacts.filter((impact) => impact.geolocation == undefined)
...
function _exercise13(impactSize) {
  ...
  return impacts.filter((impact) => impact.mass > impactSize)
                .map((impact) => impact.name);
}

function _exercise14(age) {
  // return an array with the names of all the impacts that happened after a certain year
  return impacts.filter((impact) => new Date(impact.year).getFullYear() > age)
                .map((impact) => impact.name);
}
...
function exercise15() {
  ...
  return impacts.filter((impact) =>  new Date(impact.year).getFullYear() > 1945)
                .reduce(function(acc, current){
                  let year = new Date(current.year).getFullYear();
                  acc[year] = acc[year] == undefined ? 1 : acc[year] + 1 ;
                  return acc;
                }, {});
}
